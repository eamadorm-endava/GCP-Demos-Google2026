...
import {normalizeForMarkdown} from '@/utils/text';
  ...
  return (
    <div className="w-full my-1 justify-start">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-semibold text-brand-dark">{appConfig.name}</span>
      </div>
      <div className="ml-10 px-4 py-3 rounded-2xl shadow-sm bg-solidBlue-10 text-brand-dark self-start inline-block">
        <div className="flex items-center space-x-2 h-5">
          <span className="h-2 w-2 bg-solidBlue-50 rounded-full animate-bounce [animation-delay:-0.3s]"></span>
          <span className="h-2 w-2 bg-solidBlue-50 rounded-full animate-bounce [animation-delay:-0.15s]"></span>
          <span className="h-2 w-2 bg-solidBlue-50 rounded-full animate-bounce"></span>
        </div>
      </div>
    </div>
  );
  ...
  if (isUser) {
    return (
      <div className="flex w-full my-1 items-start gap-2 justify-end">
        <div className="max-w-xs md:max-w-md lg:max-w-2xl px-4 py-2 rounded-2xl shadow-sm bg-brand-primary text-white self-end">
          <Markdown text={normalizeForMarkdown(message.text)} />
        </div>
  ...
  return (
    <div className="w-full my-1 justify-start">
      ...
      <div className="ml-10 flex-grow min-w-0">
        {message.text && (
          <div className="max-w-xs md:max-w-md lg:max-w-2xl px-4 py-2 rounded-2xl shadow-sm bg-solidBlue-10 text-brand-dark self-start inline-block">
            <Markdown text={normalizeForMarkdown(message.text)} />
          </div>
        )}
