...
function App() {
  ...
  const handleAddToCheckout = (productToAdd: Product) => {
    ...
    handleSendMessage(actionPayload, {
      isUserAction: true,
      userText: `Added ${normalizeForDisplay(productToAdd.name)} to checkout`,
    });
  };
  ...
  const handleSendMessage = async (
    ...
    if (!options?.suppressUserMessage) {
      const textForUser = options?.isUserAction
        ? options?.userText ?? '<User Action>'
      ...
      if (textForUser) {
        setMessages((prev) => [
          ...prev,
          createChatMessage(Sender.USER, textForUser),
        ]);
      }
    }
    setMessages((prev) => [
