...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (Date.now() - lastActivity > IDLE_TIMEOUT && !currentVertical) {
        setIdle(true);
      }
    }, 1000);
    ...
    <div className="h-screen w-screen flex flex-col bg-endava-dark text-white relative overflow-hidden select-none">
      ...
      <main className="flex-grow relative overflow-hidden z-10">
        ...
        {currentVertical && (
          <button
            onClick={handleSwitch}
            className="absolute top-1 right-1 z-50 p-1.5 text-white/30 hover:text-white hover:scale-110 active:scale-95 transition-all duration-300 group"
            title="Back to Gallery"
          >
            <X className="w-6 h-6 group-hover:rotate-90 transition-transform duration-500" strokeWidth={1.5} />
          </button>
        )}
      </main>
