  ...
  if (loading) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center bg-endava-dark relative overflow-hidden">
        <div className="absolute top-1/4 -left-20 w-96 h-96 bg-endava-orange/10 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-1/4 -right-20 w-96 h-96 bg-endava-orange/5 rounded-full blur-[120px] animate-pulse delay-700" />
          ...
          <div className="relative mb-12">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative">
                <div className="absolute inset-0 bg-endava-orange blur-2xl opacity-20 animate-pulse" />
                <div className="relative w-16 h-16 md:w-24 md:h-24 bg-endava-blue-90 border border-white/10 rounded-3xl flex items-center justify-center shadow-2xl">
                  {progress < 30 && <Globe className="w-8 h-8 md:w-12 md:h-12 text-endava-orange animate-bounce" />}
                  {progress >= 30 && progress < 60 && <Database className="w-8 h-8 md:w-12 md:h-12 text-endava-orange animate-pulse" />}
                  {progress >= 60 && progress < 90 && <Cpu className="w-8 h-8 md:w-12 md:h-12 text-endava-orange animate-spin" />}
                  {progress >= 90 && <Zap className="w-8 h-8 md:w-12 md:h-12 text-endava-orange animate-ping" />}
                </div>
              </div>
            </div>
            <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-endava-dark px-4 py-1 border border-white/10 rounded-full">
  ...
  if (verticalConfig?.externalUrl) {
    return (
      <div className="h-full w-full flex flex-col animate-in fade-in duration-700">
        <div className="bg-endava-blue-90 px-6 py-2 border-b border-white/5 flex items-center justify-between text-[10px] font-black tracking-[0.2em] text-white uppercase">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-3 h-3 text-endava-orange" /> Endava Sandboxed Session
          </div>
