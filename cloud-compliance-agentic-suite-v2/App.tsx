  ...
  return (
    ...
    <div className="flex flex-col h-screen bg-brand-dark text-brand-text overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {/* MAIN CONTENT AREA */}
        <main className="flex-1 flex flex-col overflow-hidden bg-brand-dark relative z-0">
          
          {/* Context Header (Breadcrumbs & User Profile) */}
          <header className="h-16 bg-brand-secondary/30 backdrop-blur-sm border-b border-brand-border flex items-center justify-between px-8 shrink-0">
            <div className="flex items-center gap-4">
              <h2 className="text-lg font-bold text-white capitalize flex items-center gap-2">
                <span className="text-brand-muted font-medium text-sm">Console</span>
              ...
              <div className="flex gap-2">
                {REGULATORY_SCOPE.map(reg => (
                  <span key={reg.category} className="px-2 py-0.5 bg-brand-dark/50 text-[10px] font-bold text-brand-muted rounded border border-brand-border/50 uppercase tracking-tighter">
                    {reg.category}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center gap-6">
              <div className="relative group">
                <i className="fas fa-bell text-brand-muted group-hover:text-brand-primary transition-colors cursor-pointer text-lg"></i>
                <span className="absolute -top-1 -right-1 w-2 h-2 bg-signal-red rounded-full border border-brand-secondary"></span>
              </div>
              <div className="flex items-center gap-3 bg-brand-dark px-3 py-1.5 rounded-full border border-brand-border cursor-pointer hover:border-brand-primary transition-colors">
                <img src="https://picsum.photos/32/32" className="w-7 h-7 rounded-full border border-brand-primary" alt="Avatar" />
                <div className="hidden sm:block">
                  <p className="text-xs font-bold text-white leading-none">Senior Auditor</p>
                  <p className="text-[9px] text-brand-muted font-medium">Regional Compliance</p>
                </div>
              </div>
            </div>
          </header>
