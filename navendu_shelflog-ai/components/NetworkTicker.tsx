...
export const NetworkTicker = () => {
    ...
    return (
        <div className="w-full bg-[#192B37] border-b border-[#47555F] h-8 overflow-hidden flex items-center relative z-20">
            <div className="bg-[#FF5640] px-3 h-full flex items-center z-10 font-bold text-[10px] text-white uppercase tracking-widest shadow-lg">
            ...
            <div className="flex whitespace-nowrap overflow-hidden flex-1 mask-linear-fade">
                <div
                    className="flex items-center space-x-12 px-4 animate-ticker text-xs font-mono text-[#8C959B]"
                    style={{ animation: 'ticker 60s linear infinite' }}
                >
                    {[...messages, ...messages, ...messages].map((msg, i) => (
                        <span key={i} className="flex items-center">
                            <span className="w-1.5 h-1.5 bg-[#47555F] rounded-full mr-3"></span>
                            {msg}
                        </span>
                    ))}
                </div>
