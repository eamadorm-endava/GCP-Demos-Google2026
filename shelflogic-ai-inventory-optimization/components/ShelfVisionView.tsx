...
import { Camera, CheckCircle, AlertTriangle, Shield, RefreshCw, LayoutGrid, Sparkles, Activity, Map, Eye, X, StoreIcon } from './Icons';
import { ALL_STORES } from '../constants';
import { theme } from '../theme';
import { generatePlanogramImage, generateLiveStoreFeed } from '../services/geminiService';
import { ShelfVisionSidebar, ShelfNode } from './ShelfVisionSidebar';
...
export const ShelfVisionView: React.FC = () => {
    ...
    const handleImageError = (type: 'ref' | 'live') => {
        if (type === 'ref') {
            setPlanogramSrc(activeNode.imageRef);
        } else {
            setLiveFeedSrc(activeNode.imageLive);
        ...
        <div className="flex flex-col md:flex-row h-auto md:h-screen md:overflow-hidden">
            ...
            <div className={`flex-1 overflow-y-auto ${theme.colors.background.main} p-4 md:p-8 h-full`}>
                <header className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
                    <div>
                        <div className="flex items-center space-x-3 mb-1">
                            <div className="flex items-center text-[#8C959B] text-xs font-bold uppercase tracking-wide bg-[#30404B]/50 px-2 py-1 rounded border border-[#5E6A73]/50">
                                <StoreIcon className="w-3 h-3 mr-1.5" />
                                {activeStore?.name}
                            </div>
                            <span className="text-[#5E6A73]">/</span>
                            <span className="text-[#FF5640] text-xs font-bold uppercase tracking-wide">{activeNode.category}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-4 self-start md:self-auto">
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`w-12 h-6 rounded-full p-1 transition-colors ${showHeatmap ? 'bg-[#FF5640]' : 'bg-[#47555F]'}`}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-8">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xs font-bold text-[#FF5640] uppercase tracking-widest flex items-center">
                                <LayoutGrid className="w-4 h-4 mr-2" />
                                Master Reference Model
                            </h2>
                        </div>
                        <div className={`${theme.components.card} p-1 overflow-hidden relative min-h-[300px] md:min-h-[450px] bg-[#30404B] border-dashed border-2 border-[#FF5640]/20 flex items-center justify-center`}>
                            <div className="relative w-full h-full">
                                {isPlanogramLoading ? <ImageSkeleton /> :
                                    <img
                                        src={planogramSrc}
                                ...
                                {!isPlanogramLoading && !scanning && activeNode.status === 'VIOLATION' && (
                                    <div className="absolute top-[30%] left-[40%] w-[180px] h-[120px] border-2 border-emerald-500/50 rounded-lg bg-emerald-500/5 border-dashed flex items-center justify-center">
                                        <div className="bg-emerald-600/90 text-white text-[9px] font-bold px-1.5 py-0.5 absolute -top-3 left-1/2 transform -translate-x-1/2 uppercase backdrop-blur-md rounded border border-[#5E6A73] shadow-lg">
                                            EXPECTED: {activeNode.sku || 'ITEM'}
                                        </div>
                                    </div>
                                )}
                            </div>
                            <div className="absolute top-4 left-4 bg-black/60 backdrop-blur px-2 py-1 rounded text-[10px] text-white font-mono border border-[#47555F]">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className={`${theme.components.card} p-1 overflow-hidden relative min-h-[300px] md:min-h-[450px] bg-slate-900 flex items-center justify-center border-2 
                            ...
                            <div className="relative w-full h-full">
                                ...
                                {showHeatmap && (
                                    <div className="absolute top-4 left-4 bg-black/60 backdrop-blur px-2 py-1 rounded text-[10px] text-white font-mono border border-[#47555F] z-20 flex items-center">
                            ...
                            {scanning && (
                                <div className="absolute inset-0 bg-[#FF5640]/10 flex items-center justify-center backdrop-blur-sm z-20">
                                    <div className="w-full h-1 bg-[#FF5640] absolute animate-[scan_2s_ease-in-out_infinite] shadow-[0_0_15px_rgba(255,86,64,0.8)]"></div>
                                    <div className="bg-black/80 px-6 py-3 rounded-2xl border border-[#FF5640]/30 text-[#FF5640] font-mono text-xs flex flex-col items-center">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className={`${theme.components.card} p-6 bg-[#FF5640]/5 border-[#FF5640]/20 flex flex-col justify-between`}>
                        <div>
                            <Sparkles className="w-6 h-6 text-[#FF5640] mb-3" />
                            <h4 className="font-bold text-white text-sm mb-1">Neural Vision Engine</h4>
                            <p className="text-[11px] text-[#A3AAAF] leading-relaxed">
                                System uses generative AI to compare real-world pixel streams against synthetic ground-truth models.
                            </p>
                        </div>
                        <div className="pt-4 mt-4 border-t border-[#FF5640]/20 flex justify-between items-center text-[10px] font-mono text-[#FF5640]">
                            <span>LATENCY: 450MS</span>
                            <span className="flex items-center">
                                <div className="w-1.5 h-1.5 bg-[#FF5640] rounded-full mr-2 animate-pulse"></div>
                                AI ACTIVE
                            </span>
                        </div>
                    </div>
