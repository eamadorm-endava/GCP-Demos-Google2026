...
import type { Message, RiskAnalysisResponse, Shipment, ShipmentSummary, CustomsDeclaration } from "../types";
...
export async function generateCustomsDeclaration(shipment: Shipment, farmName: string, language: 'en' | 'es-LA' | 'pt-BR'): Promise<CustomsDeclaration> {
  if (!API_KEY) {
    ...
    return {
        ...
        exporter: farmName,
        importer: shipment.customer,
        reason: 'Default mock data used (No API Key)'
    };
  }
  ...
  const prompt = `You are an expert customs broker. Create a customs declaration dataset for the shipment described below. 
  Determine the most likely Harmonized System (HS) Code for the commodity. 
  Estimate a realistic gross weight in KG if not provided (assume typical air freight flower shipment).
  Provide a brief reason for the HS code selection.
  ...
  try {
      const response = await ai.models.generateContent({
          ...
          config: {
              ...
              responseSchema: {
                  type: Type.OBJECT,
                  properties: {
                      description: { type: Type.STRING, description: "Detailed commercial description of goods" },
                      hsCode: { type: Type.STRING, description: "Harmonized System Code" },
                      value: { type: Type.NUMBER, description: "Total value in USD" },
                      weight: { type: Type.NUMBER, description: "Gross weight in KG" },
                      exporter: { type: Type.STRING },
                      importer: { type: Type.STRING },
                      reason: { type: Type.STRING, description: "Explanation for HS Code classification" }
                  }
              }
          }
      });
      ...
      const jsonString = response.text.trim();
      const result = JSON.parse(jsonString);
      return result as CustomsDeclaration;

  } catch (error) {
      console.error("Error generating customs declaration:", error);
      throw new Error("Failed to generate customs declaration.");
  }
}