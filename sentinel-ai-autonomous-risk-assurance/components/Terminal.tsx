...
const Terminal: React.FC<TerminalProps> = ({ logs, status, controlName }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-endava-dark font-mono text-sm relative">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2 bg-endava-dark/80 border-b border-white/10 shrink-0 z-20">
        <div className="flex items-center space-x-2">
          <TerminalIcon className="w-4 h-4 text-emerald-500" />
          <span className="text-endava-blue-20 font-medium">AGENT_SESSION_ID: {btoa(controlName).substring(0, 8)}</span>
        </div>
        <div className="flex items-center space-x-2">
          <span className={`flex h-2 w-2 rounded-full ${status === AgentStatus.EXECUTING ? 'bg-emerald-500 animate-pulse' : 'bg-endava-blue-50'}`}></span>
          <span className="text-xs text-endava-blue-40 uppercase">{status}</span>
        </div>
      </div>
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        >
          {logs.length === 0 && (
            <div className="text-endava-blue-40 italic">Waiting for agent deployment...</div>
          )}
          ...
          {logs.map((log, index) => (
            <div key={index} className="flex space-x-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="shrink-0 mt-0.5">
                ...
                {log.status === 'error' && <AlertTriangle className="w-4 h-4 text-endava-orange" />}
                {log.status === 'info' && <Activity className="w-4 h-4 text-blue-500" />}
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-endava-blue-50">[{log.timestamp}]</span>
                  <span className={`font-semibold ${log.status === 'error' ? 'text-endava-orange' : 'text-emerald-500 terminal-text'
                    }`}>
                    {log.action}
                  </span>
                </div>
                <p className="text-endava-blue-30 text-xs mt-1 pl-1 border-l-2 border-white/10 leading-relaxed">{log.detail}</p>
              </div>
            </div>
          ))}
        </div>
