...
const RiskDashboard: React.FC<RiskDashboardProps> = ({ risks, activeSessions, onDeployAgent, onStopAgent, completedAudits, onReset, onAddRisk }) => {
   ...
   return (
      <div className="flex h-screen bg-endava-dark text-white font-sans overflow-hidden">
         ...
         {selectedAgent && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-endava-dark/90 backdrop-blur-md p-4 animate-in fade-in zoom-in-95 duration-200">
               <div className="bg-endava-blue-90 border border-white/10 rounded-xl max-w-5xl w-full shadow-2xl overflow-hidden flex flex-col h-[90vh]">
                  <div className="p-6 border-b border-white/5 flex items-center justify-between bg-endava-dark/50 shrink-0">
                     <div className="flex items-center space-x-3">
                        ...
                        <div>
                           ...
                           <div className="text-xs text-endava-blue-40 font-mono flex items-center space-x-2">
                              <span>AGENT_ID: {selectedAgent.id}</span>
                              <span>•</span>
                              <span className="text-endava-orange">v2.4.0-STABLE</span>
                           </div>
                        </div>
                     </div>
                     <button onClick={() => setSelectedAgentId(null)} className="text-endava-blue-50 hover:text-white transition-colors">
                        <X className="w-6 h-6" />
                     </button>
                  </div>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col md:flex-row">
                     {/* Sidebar Info */}
                     <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-white/5 p-6 bg-endava-blue-90/50 order-2 md:order-1 overflow-y-auto">
                        <div className="mb-6">
                           <div className="text-xs font-semibold text-endava-blue-50 uppercase tracking-wider mb-2">Capabilities</div>
                           <ul className="space-y-2">
                              {selectedAgent.capabilities.map((cap: string) => (
                                 <li key={cap} className="text-sm text-endava-blue-30 flex items-start">
                                    <div className="w-1.5 h-1.5 rounded-full bg-endava-orange mt-1.5 mr-2 shrink-0"></div>
                                    {cap}
                                 </li>
                              ))}
                           </ul>
                        ...
                        <div className="mb-6">
                           <div className="text-xs font-semibold text-endava-blue-50 uppercase tracking-wider mb-2">Connected Systems</div>
                           <div className="flex flex-col space-y-2">
                              {selectedAgent.requiredIntegrations?.map((intId: string) => {
                                 ...
                                 return (
                                    <div key={intId} className="flex items-center justify-between px-3 py-2 rounded bg-endava-dark border border-white/5 text-xs text-endava-blue-30 w-full">
                        ...
                        <div className="mb-6">
                           <div className="text-xs font-semibold text-endava-blue-50 uppercase tracking-wider mb-2">Model Spec</div>
                           <div className="p-3 bg-endava-dark rounded border border-white/5 text-xs font-mono text-endava-blue-40 space-y-2">
                              <div className="flex justify-between">
                                 <span>Base:</span> <span className="text-white">{selectedAgent.model}</span>
                              </div>

                              {selectedAgent.config ? (
                                 <>
                                    <div className="flex justify-between items-center pt-2 border-t border-white/5/50">
                                 ...
                                 <>
                                    <div className="flex justify-between">
                                       <span>Temp:</span> <span className="text-white">0.2</span>
                                    </div>
                                    <div className="flex justify-between">
                                       <span>Context:</span> <span className="text-white">1M</span>
                                    </div>
                                 </>
                              )}
                           </div>
                     ...
                     <div className="flex-1 bg-[#1e1e1e] flex flex-col min-h-0 order-1 md:order-2 overflow-hidden">
                        {/* Tabs */}
                        <div className="flex items-center bg-[#252526] text-xs text-endava-blue-40 border-b border-white/10 select-none shrink-0">
                           <button
                              onClick={() => setActiveTab('logic')}
                              className={`px-4 py-2 flex items-center space-x-2 border-t-2 transition-colors ${activeTab === 'logic'
                                 ? 'bg-[#1e1e1e] text-endava-orange border-blue-500'
                           ...
                           <button
                              ...
                              className={`px-4 py-2 flex items-center space-x-2 border-t-2 transition-colors ${activeTab === 'prompt'
                                 ? 'bg-[#1e1e1e] text-endava-orange border-blue-500'
                                 : 'hover:bg-[#2a2d2e] border-transparent'
                                 }`}
                           >
                        ...
                        <div className="flex-1 flex overflow-hidden font-mono text-sm leading-6">
                           {/* Line Numbers */}
                           <div className="w-12 bg-[#1e1e1e] text-endava-blue-60 text-right pr-3 pt-4 select-none border-r border-white/5 shrink-0 overflow-hidden">
                           ...
                           {/* Code Content */}
                           <div className="flex-1 overflow-auto p-4 pt-4 text-endava-blue-30">
                  ...
                  <div className="p-4 border-t border-white/5 bg-endava-blue-90 flex justify-end space-x-3 shrink-0">
                     <button onClick={() => setSelectedAgentId(null)} className="px-4 py-2 rounded-lg text-sm text-endava-blue-40 hover:text-white hover:bg-endava-dark transition-colors">Close</button>
                     <button
                        onClick={() => {
                           setSelectedAgentId(null);
                           setViewMode('ops');
                        }}
                        className="px-4 py-2 rounded-lg text-sm bg-endava-dark hover:bg-endava-orange text-white font-medium shadow-lg shadow-endava-orange/20"
                     >
                        Deploy Agent
                     </button>
                  </div>
