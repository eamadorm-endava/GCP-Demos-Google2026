    ...
    <div className="flex-1 overflow-y-auto p-8 bg-endava-dark">
      <header className="mb-8 flex items-center justify-between">
         <div>
           ...
           <p className="text-endava-blue-40">Available autonomous agents deployed for risk verification and assurance tasks.</p>
         </div>
         <button 
            onClick={onAddAgent}
            className="flex items-center space-x-2 px-4 py-2 bg-endava-dark hover:bg-endava-orange text-white rounded-lg text-sm font-medium transition-colors shadow-lg shadow-endava-orange/20"
         >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-6 pb-12">
         {agents.map((agent) => (
            <div key={agent.id} className={`group relative bg-endava-blue-90 border ${agent.border} rounded-2xl p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1`}>
               {/* Header */}
               <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                     <span className="px-2 py-1 rounded text-[10px] font-mono bg-endava-dark text-endava-blue-40 border border-white/10">v2.4.0</span>
                     <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                  </div>
               </div>
               
               <h3 className="text-xl font-bold text-white mb-2">{agent.name}</h3>
               <p className="text-endava-blue-40 text-sm leading-relaxed mb-6 h-16 line-clamp-3">{agent.description}</p>
               
               {/* Tech Specs */}
               <div className="flex items-center space-x-4 mb-6 text-xs text-endava-blue-50 font-mono">
               ...
               <div className="mb-6">
                  <div className="text-xs font-semibold text-endava-blue-50 uppercase tracking-wider mb-3 flex items-center">
                  ...
                  <div className="flex flex-wrap gap-2">
                     {agent.requiredIntegrations?.map((intId: string) => {
                       ...
                       return (
                        <div key={intId} className="flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-endava-dark border border-white/5 text-xs text-endava-blue-30">
                     ...
                     {(!agent.requiredIntegrations || agent.requiredIntegrations.length === 0) && (
                        <span className="text-xs text-endava-blue-60 italic">No integrations configured</span>
                     )}
                  </div>
               </div>
               ...
               {/* Action */}
               <div className="mt-8 pt-6 border-t border-white/5/50 flex justify-end">
                  <button 
